---
scope: L3
summary: "Backend implementation detail: how the /v1 Axum adapter implements the API contract over the RuntimeHandle (handler patterns, error mapping, route guard), the runtime/sync/mutation/event flows, and the extension points for new providers, reads, mutations, and events. Transitional wrapper-migration controls live in eph/PLAN-L3-api-runtime-wrapper-migration."
modified: 2026-06-24
reviewed: 2026-07-01
state: realized
depends:
...
| message-mutations-runtime-backed | MUST | Message send/keyword/mailbox/destroy/detail/attachment routes and the session-scoped `run_mutation` call `RuntimeHandle` methods; list/search routes use `query_mail_page` rather than route-shaped methods. |
