...
## Survey findings → unit shaping (2026-07-03)
- **Ruling 22 is a COMPLETION**: apps/mcp already ships a real MCP server (10 tools, @modelcontextprotocol/sdk, stdio). Remaining: (b) subscription — add a notifications capability + forward the existing cli/events.ts SSE-tap consumer as MCP notifications; a dedicated reply tool; connect-time per-connection token mint (the mint pipeline exists, only the call-site in index.ts is missing).
- **Ruling 23 prerequisites (before the GUI pane)**: (1) **STRUCTURAL exec-exclusion** — introduce `WritableRuleAction` (RuleAction minus Exec) as the write-route body type so exec is UNREPRESENTABLE at the serde boundary (today the RCE invariant is protected only by the absence of a write route — a latent trap: RuleAction is one flat enum used for both file parse and any future POST). (2) **Rules reload path** — rules load once at bootstrap (build.rs spawn_rule_engine); a GUI/REST-created rule would list but NOT RUN until restart. A write route MUST also trigger a rule-engine reload. (3) **Persistence boundary** — prefer a GUI-owned rules.d/ merged at load (or a separate store) over splicing hand-authored rules.toml; if writing rules.toml, use the write_managed_toml/toml_edit atomic pattern + file locking (neither exists yet). The WHEN-clause GUI (RuleGroupEditor) already exists for the shared grammar; the action-picker + these three server-side prerequisites are the work.
...
DEFERRED: the Linux AppImage sidecar — linuxdeploy fails on the Bun --compile binary even with NO_STRIP (needs a local x86_64 repro), and the AppImage layout lacks POSTHASTE_APP_DIR wiring to make the sidecar discoverable anyway; linux users use the wizard tarball path. Follow-up: repro + fix linuxdeploy, wire POSTHASTE_APP_DIR in the app's env, then re-enable externalBin for linux. Windows follow-up: verify the NSIS install path against ctl.rs's candidate list on a real Windows machine.
