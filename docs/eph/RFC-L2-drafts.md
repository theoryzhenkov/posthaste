...
## M65/M66 design refinement (2026-07-04 — investigation before code; two load-bearing facts)
The send/save worker studied the M64 template + the save/send paths and found "mirror M64" is UNSAFE for send:
- **The fold vocabulary can't UPSERT.** MessageAssertion (replica-core message.rs:138) = SetKeywords|ReplaceMailboxes|Destroy|ApplyDiff; MessageFoldState carries only {keywords, mailbox_ids}. It cannot CREATE a row or carry draft content. So SaveDraft/Send optimistic-UPSERT is NOT expressible as a fold. **M65 (save): fold_effect = None** + the row appears via a D132 message.updated{upsert-projection} emitted on DraftCreate/DraftUpdate settlement (read back post-reconcile, since save rotates ids). Still the real win: the typed idempotent runMutation path replaces the fire-and-forget POST.
- **Send settlement is ASYNC and unbridged.** forward_mutation_for (commands.rs:472) emits Settlement{Confirmed} at ENQUEUE (apply_operation Ok). A send's real outcome (ack/park/fail) happens LATER at flush (outbox.rs:560-746) as operation.settled / operation.dispatch_uncertain DOMAIN events with NO client_mutation_id linkage. A naive Send+Destroy-fold via runMutation → settles Confirmed at enqueue → retires the Destroy → the draft REAPPEARS (server defers consume to ack, outbox.rs:671) → blinks out on ack; and an async park/fail produces NO mutation rejection → the revert/keep-draft NEVER fires = flicker + silent-failure reintroduction on the beta-critical send path.
- **D125/D126 already correct server-side (M60):** send consumes the draft ONLY on ack (consume_draft_after_send); a parked DispatchUncertain send KEEPS the draft.

**New requirement — the async-settlement→mutation BRIDGE (the real M66):** carry the send's client_mutation_id on its outbox Operation; the operation's TERMINAL FLUSH settlement emits a Settlement/mutationNotification frame to the origin runtime — Applied→confirm (draft consumed, Sent confirmed), DispatchUncertain→REVERT (draft returns, park UI, NO false Sent), Failed→revert+surface. Cross-cutting: the operation model (client_mutation_id field), emit_settlement/emit_dispatch_uncertain (outbox.rs:1048), near-node frame routing, entityStoreAdapter settle/revert. This is the correct, general mechanism for any async-settled op — distinct from the M64 synchronous-destroy template. Idempotency preserved: apply_operation reserves under op.name()=SEND_OP_NAME='message.send' (ruling 24).
Sequencing (ONE worker, sequential — all touch outbox.rs/commands.rs, cannot parallelize): M65 (save, safe) → the send-settlement bridge → M66 (send on the bridge). Tested via the M48 harness (no real WS — M67 owns that).
