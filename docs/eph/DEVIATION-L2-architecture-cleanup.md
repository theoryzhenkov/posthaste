...
| V4 | D10, D13 | backend/L2; architecture/L2-crate-topology §1.1–1.2 | `posthaste-domain` god-crate: 13 modules, model+ports+MailService+provider+imap+cache+search, `mail-parser`+`tokio` deps; `ConfigError` in `config.rs`, `ValidationError` in `validation.rs`; every wire consumer drags all of it | `posthaste-domain-model` (pure types incl. relocated `ConfigError`/`ValidationError`, per RFC §6.1 file assignment) + `posthaste-domain-service` (hexagonal core); no glob re-export shim in end-state | open |
...
| V6 | D15 | architecture/L2-crate-topology §3 | wasm-pure frontier exists by accident, no CI check; already breached via `link-wasm → runtime-contract → domain → {mail-parser, tokio}` | `link-core`, `link-replica`, `domain-model`, `contract-core` build for `wasm32-unknown-unknown` in CI | open |
