...
| D9  | Extract explicit `Replica` (and possibly `Link` / `Outbox`) **trait seams** over the already-unified `MessageReplica`, so the Link/Replica/Outbox layering is legible in types (it is ~80% realized in structure today). | XXII; XV | drained |
...
| R1  | **Rejected:** `posthaste-authority-server-api` (typed backend trait). `BackendApi` is all-link (no wire-free RPC behind it) and `subscribe → DownStream` returns a wire frame; the trait + wire go together. A typed backend would also force the runtime to decode+dispatch the mail vocabulary in a *second* place. | XIV (would duplicate dispatch); XX; keeps the opaque relay | rejected |
...
| D26 | **Delete the never-consumed tuning surface**: `DaemonRuntimeTuning` + 5 sub-structs (`config/runtime.rs`, 142 LOC, exported `lib.rs:13-16`, zero workspace consumers; doc admits "so call sites can be wired incrementally" — a seam with no named second thing). Re-add when a consumer exists. | XX; XXI | drained |
...
| `BackendApi` | `AuthorityServerApi` | trait | posthaste-authority-server-link | drained |
| `BackendLink` | `AuthorityServerLink` | type | posthaste-authority-server-link | drained |
...
- `imap/types.rs`, `imap/sync_state.rs`
- `provider/kind_profile.rs` (`ProviderKind`, `ProviderProfile`)
...
| M5 | **Type the operation vocabulary**: `MessageMutation` moves into `contract-core` and becomes `MailOperation` **including the `revCursor` control arm (D22)**; `MutationRequest` carries it typed; dispatch becomes an exhaustive match at exactly the §6.6 crossings (W1/W1b/W2; the in-process string re-parses collapse); the 5 per-command RPCs collapse into `apply_mail_operation` and REST-retry idempotency is verified/added (D21); delete `WireMutationId`; the settlement arms are already trimmed in specs (D16 — verify code matches: live set is `Accepted`/`Confirmed`/`Failed`); assert Q4's dep-drop (`link-wasm` → `link-core + link-replica + contract-core`). `backend.rs` splits (reads/commands/pub-sub) while its dispatch is rewritten (D29). | D8, D11, D12, D16, D21, D22, D29 | V2 (closes) | The root fix of the "dirtiness"; last of the topology moves because it touches every wire crossing. |
| M6 | **Replica trait seams**: explicit `Replica` (and if warranted `Link`/`Outbox`) trait views over the single owner; lift the version-gated retire invariant from `EntityStore::settle` into the engine seam (its second consumer, the runtime near node, exists now) | D9 | V7 | Semantics unchanged (R2: one owner, two views). |
