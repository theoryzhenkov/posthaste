...
| M10 *(candidate, next wave)* | **Fold the two remaining down-channels onto the shared machinery**: `/v1/events` (own afterSeq replay + N8 silent Lagged drop) and the sessionless `/v1/views/{id}/stream` pair (kept at M3's D23 verdict, never revisited) both hand-roll seq/lag/reconnect beside the engine+far-end stores — the D50 pattern, two more instances. Fold onto far-end stores + engine, or ratify separate-by-design with reasons; evaluate dissolving the sessionless pair into session views now that reconnect is engine-cheap. | (to ratify) | — | Registered from the 2026-07-02 discrepancy sweep. |
