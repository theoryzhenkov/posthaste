...
| M6 | **`OptimisticReplica` trait seam (D35a)**: extract the trait in link-core over the single owner; `EntityStore` and the runtime outbox implement it; lift the version-gated retire invariant from `EntityStore::settle` into the engine seam (its second consumer, the runtime near node, exists now) | D9, D35 | V7 | Semantics unchanged (R2: one owner, two views). |
