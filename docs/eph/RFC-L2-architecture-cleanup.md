...
| M10 *(reshaped 2026-07-02)* | **Sessionless-views deletion + riders only** — D51's delete half: remove the two HTTP handlers + dead TS, migrate testkit's in-process use to the link-scoped path, retire open_view/subscribe_view, fix the dangling DESIGN-L2-posthastectl references + the openMessageListView naming collision. D51's REMOUNT half (/v1/events → tap) TRANSFERRED to RFC-L2-scripting S2 (deferred with that RFC). | D51 | — | Small unit; runs whenever the cleanup track resumes. |
