...
| M23b | **Port-boundary offload adoption**: route production store callers through M23's async seam — decide async port traits vs concrete `Arc<DatabaseStore>` threading for the hot paths (`ServiceSyncSink::emit` first); small design pass THEN execution (the M23 agent's flag — the seam exists, adoption is consumer-facing). | D63 (completion) | Gate: the sync hot path demonstrably runs on the blocking pool (tokio-console or a thread-name assert in test). |
...
| M27 | **Bounded growth batch** *(M26 flag: sub-unit (d)'s progress-spawn fix must also GENERATION-GATE the writes — a late progress write from an arm-abandoned cycle observably flaps status Degraded→Syncing; reuse the supervisor's existing generational-guard vocabulary)* (independently landable sub-units a–e). | D67 (N12/N15/N16/N5/N6) | Gate: flood tests bound memory; large-outbox read is paginated. Grep: `rg 'LIMIT' crates/posthaste-store/src/{outbox,snooze}.rs` ≥1; a `flow_store` sweep task exists; the TLS `JoinSet` has a cap constant. |
