...
| **RFC-L2-view-membership-negotiation** | One predicate, two evaluators, runtime-owned assignment: the caller declares an evaluation capability (D176), the runtime returns each view's membership contract as data (`ClientMaintained{predicate}`/`RuntimeServed`, D177), mismatches fail toward re-serve not staleness (D178), and the smart-mailbox rule AST compiles to a projection-decidable client IR so user views self-maintain when provable (D179). Retires the client-stamped `client_self_maintained` flag. | **PROPOSED (2026-07-16)** — Slice 0 (testkit `MailListMirror`, the contract's client half) landed; Slices 1–5 (contract + runtime decision, client adoption, flag retirement, compiler-proven predicates, L3 gate graduation) not started. |
