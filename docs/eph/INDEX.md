...
| **RFC-L2-client-replication-model** | The north-star: `visible = fold(base, intent_log)`, one writer per plane, mutations-as-single-intents, verdict as projection — enforced by a Rust type-system `BaseWrite` capability (Tier 1). §6: the base/overlay/effective substrate (D167–D169) — optimism materialized as the fold's *output* into a `message_overlay` table, every SQL read via the `_effective` view; SQL stays the single predicate engine, replica-core the single fold (one engine, two storage backends). Parent of the send-RFC and the test-taxonomy. | **PROPOSED / DESIGN — not implemented** — NS1 overlay substrate (strangle SQL reads → delete the S2 write-through → collapse `_protected`) → NS1b `BaseWrite` seal capstone → NS2 send-as-intent → NS3 generalize. Tier-2 linters deferred. |
| **RFC-L2-send-draft-state-machine** | Collapse the draft/send *shadow* state machines back into their typed owners: scheduling-as-state + the undo-send clock fix (a live P0: nothing sends in nightly), `DraftRegistry` as sole identity authority, a complete `SendOutcome` (kills `moved_to_sent`), typed+versioned persistence. Child of the replication-model RFC (its NS2). | **PROPOSED / DESIGN — not implemented** — Phase 1 (M80 clock fix → M81 scheduling-state) restores send; M82 identity; M83 outcomes; M84 schema-versioning gates persistence. |
