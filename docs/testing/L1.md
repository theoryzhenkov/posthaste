...
| `Harness::with_runtime` | Stand up a `RuntimeCore` against the same store/config so view-settlement assertions observe the runtime recompute path. | planned |
| View-settlement recorder | Capture the ordered view-diff stream from a runtime; assert convergence (no missed / over-broad recompute) via golden + invariants. | planned |
...
The settlement recorder, runtime-in-harness, declarative fixtures, and
`posthastectl` headless driver are specified in
`docs/eph/PLAN-L2-testkit-roadmap.md` and fold back here as they land.
