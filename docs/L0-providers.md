...
| OAuth token lifecycle | `posthaste_server::oauth::OAuthProviderProfile`, `OAuthTokenService`, `OAuthTokenSet`, `supervisor::resolve_account_secret` | OAuth 2.1 is still an IETF draft, so Posthaste applies its security posture over OAuth 2.0 primitives: <https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1>; RFC 8252 native app guidance uses an external user-agent, loopback redirects, and PKCE: <https://datatracker.ietf.org/doc/html/rfc8252>; RFC 9700 recommends authorization code with PKCE and deprecates weaker flows for modern clients: <https://www.rfc-editor.org/rfc/rfc9700.html>; `oauth2` crate PKCE and async request support: <https://docs.rs/oauth2/latest/oauth2/>; Google desktop OAuth, OpenID Connect identity claims, and Gmail XOAUTH2: <https://developers.google.com/identity/protocols/oauth2/native-app>, <https://developers.google.com/identity/openid-connect/openid-connect>, <https://developers.google.com/workspace/gmail/imap/xoauth2-protocol>; Microsoft OIDC scopes and IMAP/SMTP OAuth scopes: <https://learn.microsoft.com/en-us/azure/active-directory/develop/scopes-oidc>, <https://learn.microsoft.com/en-us/exchange/client-developer/legacy-protocols/how-to-authenticate-an-imap-pop-smtp-application-by-using-oauth> |
...
checks the ID-token issuer, audience, expiry, nonce, and verified-email status,
then uses the identity email to create the local IMAP/SMTP account, sender
address ownership, and XOAUTH2 username.
