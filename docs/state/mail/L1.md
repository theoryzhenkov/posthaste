---
scope: L1
summary: "Mail state ownership, canonical records, derived projections, backend-side query evaluation, conversation freshness, message change assertions, and runtime view rules"
modified: 2026-06-22
reviewed: 2026-06-22
dependents:
...
This domain is transport-neutral. API specs describe how mail state crosses HTTP and SSE. Backend specs describe how state is persisted, indexed, synced, and mutated. Runtime and client specs describe how state is operated and rendered.
...
[JMAP RFC 8620](https://www.rfc-editor.org/rfc/rfc8620) and [JMAP Mail RFC 8621](https://www.rfc-editor.org/rfc/rfc8621) are the main reference for server-owned state strings, query truth, change recovery, push notification, and fallback when incremental changes cannot be calculated. Posthaste does not expose JMAP directly to clients, but it borrows the rule that opaque server state and backend query evaluation beat client reconstruction. [Stalwart](https://github.com/stalwartlabs/stalwart) is a useful open-source JMAP implementation reference, especially its [`query_changes`](https://github.com/stalwartlabs/stalwart/blob/cd4b0854/crates/jmap/src/changes/query.rs) path.

[CQRS](https://martinfowler.com/bliki/CQRS.html) is the reference vocabulary for separating command writes from optimized read projections. Posthaste does not need event sourcing to use this split: commands and sync mutate canonical records; query endpoints read backend projections.

[SQLite query planning](https://www.sqlite.org/queryplanner.html), [SQLite optimizer behavior](https://sqlite.org/optoverview.html), and [expression indexes](https://sqlite.org/expridx.html) are the implementation references for keeping backend query reads fast. Query-scope and predicate composition belongs in store reads when representable.

## 2. State ownership

### 2.1 Authority runtime state

The authority runtime's local store is authoritative for renderer-visible mail state in bundled application mode. It is built from provider sync, provider command outcomes, local configuration, and cache fetches.

Provider state is upstream authority, but renderers do not talk to providers directly. Renderers read through the runtime boundary. External API clients read through the API and observe backend state changes through the API event stream.
...
The backend may materialize projections or indexes for performance. A materialized projection is not canonical unless this domain names it as canonical.

## 3. Canonical mail state

Canonical mail state consists of backend-owned records that cannot be derived solely from other backend records without provider input or prior writes:
...
Cached body/blob bytes are backend-owned local cache records. They may be evictable; their presence is not the same kind of canonical fact as message metadata or provider identity.
...
`bodyToken` is an opaque equality token for the message body state visible through detail reads and body-dependent search. It changes when the body text, sanitized body, draft body content, or backend body representation for the message changes.
...
`conversationId` identifies the derived group. `conversationToken` is server-owned and opaque. It changes whenever the backend's complete conversation membership or aggregate data changes in a way that can make a hydrated conversation envelope stale.
...
A query projection is a backend-owned filtered view over canonical message state. Query projections include source mailbox lists, smart-mailbox lists, global search results, conversation lists, and any future saved or ad hoc query scopes.

`QueryScope` is the normalized description of one backend-evaluated view: base scope, optional query text, sort, direction, cursor, limit, and any resource filters. The base scope decides where query text applies.

`QueryPage` is a finite response for one `QueryScope`. It contains projected items, cursor state, and the backend sequence or read watermark when available.
...
Query projection membership, ordering, page boundaries, and cursor continuity are not derived by the renderer from loaded messages. The runtime or backend settles the exact window.

## 5. Query and filter evaluation

### 5.1 Backend evaluation rule

Search strings and filter predicates are evaluated by the backend against local indexed message projections.

For example, `from:Alex` is parsed by the backend query language and evaluated against sender name and sender email fields in the local store. The browser may validate syntax, show completions, and preserve the typed query, but it does not decide the authoritative result set.

### 5.2 Scope composition

Query text is the universal selector for mail-list reads. `in:Inbox` names a saved smart mailbox query, while `in:Account/` names a concrete source and `in:Account/Archive` names a concrete mailbox under that source. If the user is in Inbox and applies `from:Alex`, the backend evaluates `in:Inbox from:Alex`. If the user switches to Archive under a source, the runtime evaluates `in:Account/Archive from:Alex`.
...
Saved smart mailboxes, source mailbox filtering, global search, conversation search, automation previews, and query-page reads use the same backend predicate semantics for the same query language.

If a query prefix, operator, or field changes, every backend query surface using that language changes together or explicitly documents a narrower subset.

## 6. Conversation model

### 6.1 Derived identity

Conversation identity is derived from provider thread identity and account/source scope when the provider supplies a thread identity. Fallback identity must be deterministic and backend-owned.

Consumers treat `conversationId` as opaque. They must not reconstruct conversation membership from the ID.

### 6.2 Freshness token

The backend computes the conversation token from complete backend knowledge of the conversation. The implementation may use a stable hash, a projection revision, an event watermark, or another opaque value.
...
State assertions are the **down-channel** of a coherent link, and named mutations are its **up-channel**; the authoritative base a link serves down carries a confirmation watermark over the consumer's mutation ids so optimistic mutations retire on confirmation rather than on send. This domain owns the assertion *shape*; [Replication](../../replication/L1.md) owns the link mechanism that moves it.
...
The runtime may compare before/after summaries to decide whether a loaded base scope can be affected. It must not use before/after summaries as the authoritative evaluator for arbitrary query text when an exact backend/store query is required.

### 7.3 Other state assertions

Backend state that affects query scopes is reported as state assertions for that state, not as query invalidation commands.
...
A `MailListViewState` is not a client-side filtered page. It is a runtime assertion of the rows currently visible or requested for one list view. Replacement rows, insertion position, row removal, cursor movement, and continuation state all come from runtime/backend query evaluation.
...
The renderer is not required to repair exact query windows locally. A single changed message can affect insertion position, page boundaries, replacement rows, and cursors; those are runtime/backend query results.
...
| ID | Sev. | Assertion |
| --- | --- | --- |
| mail-backend-authority | MUST | The authority runtime local store is authoritative for renderer-visible mail state in bundled application mode. |
...
| backend-query-evaluation | MUST | Search and filter predicates such as `from:Alex` are evaluated backend-side against local indexed projections. |
...
| shared-query-semantics | MUST | Saved smart mailboxes, source mailbox filtering, global search, conversation search, automation preview, and query pages use shared backend predicate semantics unless a narrower subset is documented. |
...
| query-window-runtime-settled | MUST | Exact query page membership, ordering, replacement rows, and cursors are settled by runtime/backend query results, not renderer repair. |
...
| no-query-invalidation-events | MUST | Backend emits state assertions, not query-scope invalidation commands. |
