...
Query text is relative to the current base scope. If the user is in Inbox and applies `from:Alex`, the backend evaluates `from:Alex` inside the Inbox scope. If the user switches to Archive while the query text remains active, the runtime opens or evaluates a different `QueryScope` for Archive plus the same query text.

Global search is a separate base scope. It is not implied by typing query text while a mailbox or smart mailbox is selected.
