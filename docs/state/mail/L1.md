...
A recipient field the authority server never parsed is repairable rather than lost: these headers are immutable per message, so any retained copy of the message source re-derives the same values, and a re-derivation may only FILL an absent field — never replace a present one, and never blank one from a source that carried no such header.
