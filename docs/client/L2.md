...
During the `runtimeMailListViews` migration path, `MessageList` opens a runtime session, opens a `mailList` view in that session, and subscribes to the unified `RuntimeFrame` stream. `viewSnapshot` and `viewReplace` frames are applied with exact `setQueryData` writes to the corresponding TanStack Query key; the legacy domain-event refetch hook is disabled for that list while the feature flag is on.
