...
*The Rust-side parse-once into the typed `MailOperation` (contract-core) has not
landed; the runtime still dispatches the wire string (RFC D8/D11).*
...
[::state partial plan=eph/RFC-L2-architecture-cleanup]
*The typed `MailOperation` vocabulary (contract-core) has not landed; the runtime
still dispatches the wire string (RFC D8/D11).*
