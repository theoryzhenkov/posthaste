...
```text
connectRenderer(init)
subscribeRuntime(afterSeq?)
openView(descriptor, options?)
subscribeView(viewId, afterRevision?)
updateView(viewId, descriptor | windowRequest)
closeView(viewId)
runMutation(name, args, clientMutationId, context?)
resolveResource(resourceDescriptor)
```

The facade returns runtime snapshots, frames, receipts, and errors in the shapes defined by the runtime spec. React hooks may wrap these calls, but components should not import transport-specific modules.
...
Hooks consume `view.snapshot`, `view.replace`, `view.error`, and `view.closed` frames. A hook may keep the latest snapshot for React rendering, but it must not repair missed frames by applying mail-specific rules. If resume fails, it resubscribes and accepts a full runtime snapshot.
...
During the `runtimeMailListViews` migration path, `MessageList` opens a runtime `mailList` view and subscribes to its dedicated view stream. `view.snapshot` and `view.replace` frames are applied with exact `setQueryData` writes to the corresponding TanStack Query key; the legacy domain-event refetch hook is disabled for that list while the feature flag is on.
...
A command hook treats `accepted`, `localApplied`, `queued`, `confirmed`, `failed`, and `conflict` as runtime facts. It may map them to toasts, row markers, disabled controls, or conflict dialogs, but it does not maintain a separate mutation replay log.
...
| view-frame-consumption | MUST | Hooks consume runtime view frames and fall back to full snapshots rather than repairing mail state locally. |
...
| no-client-mutation-outbox | MUST | Command hooks render runtime settlement state without maintaining a separate mail mutation replay log. |
