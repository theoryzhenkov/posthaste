---
title: "Client — Mail runtime (L2)"
scope: L2
summary: "Client renderer implementation structure: runtime adapter, subscriptions, view components, command dispatch, local presentation state, the worker-hosted replica plus main-thread live-store, and tests"
modified: 2026-07-04
reviewed: 2026-07-04
state: realized
depends:
  ...
  - path: docs/runtime/adapter/L2
    section: "2. Links and view registry"
    local: "3. View subscriptions"
...
This spec covers the **renderer**. When the runtime is remote, the device also runs a **client-layer link node** (a replica runtime) that holds the pending set, optimistic state, and cache and converges by confirmation watermark ([replication L1](../replication/L1.md), [runtime internals L1 §1](../runtime/internals/L1.md)); that node is a runtime implementation, not part of this spec. The renderer never runs its own optimistic mutation engine — it renders the optimistic state and pending markers its runtime node serves.
...
When the runtime node is a client-layer replica, its wasm store runs in a Web Worker and a dumb main-thread reactive store (`apps/web/src/live-store`, `useSyncExternalStore`) mirrors the worker-projected view/counts/health for React; the mirror holds no fold or projection logic ([L3 §6–§7](L3.md)).
