...
The renderer is distinct from the **client-layer link node**. When the runtime is remote, the user's device also runs a client-layer node — a replica runtime that holds an outbox, optimistic state, and a cache, and converges with the remote authority by confirmation watermark ([replication L1](../replication/L1.md), [runtime L1 §2.3](../runtime/L1.md)). That node is a runtime, not part of this domain. "The client holds local optimistic state" is true of the client-layer node; "the renderer holds no authority" is true of the view. Both hold at once.
...
In bundled mode, the adapter is backed by the embedded authority runtime. The adapter hides whether transport is in-process, IPC-backed, a contained localhost bridge, or later implemented by a local replica.
...
The UI submits named mutations with stable client mutation IDs. The runtime returns pending, settled, failed, or conflict state through the same runtime state channel used for rendering.

The UI may present undo, retry, or conflict choices exposed by the runtime. Conflict choices are submitted back as runtime mutations. Undo is itself a named mutation (the inverse assertion), not a renderer-side rollback engine.

The **renderer** does not maintain its own mail mutation outbox or optimistic projection: accepted mutation idempotency, the outbox, and settlement are runtime-node state. When the runtime is remote that outbox lives in the client-layer replica node, not in renderer view state. The renderer renders the runtime node's served optimistic state and pending markers; it does not run a second, independent optimism engine that races the served state.
...
| renderer-no-outbox | MUST | The renderer maintains no mail mutation outbox or independent optimistic projection; the outbox and optimistic state belong to its runtime node (embedded authority, or a client-layer replica when the runtime is remote). |
