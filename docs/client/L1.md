...
The client does not own authoritative mail state, mail query evaluation, provider access, mutation settlement, active query-window repair, or durable body/blob cache state.
...
[Replicache](https://doc.replicache.dev/concepts/how-it-works) is reference material for optimistic mutators and server reconciliation. The current bundled application UI uses the Posthaste runtime contract instead of adopting Replicache.
...
The UI may present undo, retry, or conflict choices exposed by the runtime. Conflict choices are submitted back as runtime mutations. The UI does not maintain a separate mail mutation outbox because accepted mutation idempotency and settlement are durable runtime state.
...
| no-client-mutation-outbox | MUST | The client does not maintain a separate mail mutation outbox; accepted mutation idempotency and settlement are runtime state. |
