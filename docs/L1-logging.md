...
Application log events use a stable structured schema so local diagnostics can be queried without reverse-engineering message text. Local logs are not telemetry payloads and must not be scraped, filtered, or uploaded by the telemetry pipeline.
...
Event names describe what happened, not where it was logged: `<domain>.<object>.<outcome>` or `<domain>.<operation>.<stage>`. Examples: `api.request.started`, `api.request.completed`, `http.request.completed`, `cache.maintenance.completed`, `sync.cycle.failed`. New user-facing or operation-relevant logs must include `event`.
...
| stable-event-field | SHOULD | New operation-relevant log events include a stable dot-separated `event` field separate from human `message` |
