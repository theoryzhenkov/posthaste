...
`entityStoreAdapter` (`apps/web/src/runtime/replica/entityStoreAdapter.ts`) is a third `RuntimeAdapter` backed by the WASM handle plus host-owned transport and persistence, installed unconditionally alongside `httpAdapter` (which serves non-replicated surfaces). The renderer is unchanged: a `mailList` view's frames are produced locally by `projectJson` after each accept/settle/ingest, while non-replicated surfaces pass through. Parity tests assert the entity-store adapter yields the same observable frames as `httpAdapter` for the covered flows.
