---
scope: L1
summary: "Coherent links: the symmetric optimistic-replication mechanism that moves mail state between layers with local optimism, an outbox, and authority-confirmed convergence, composed across client, runtime, and backend"
modified: 2026-06-24
reviewed: 2026-06-24
state: realized
depends:
...
Each arrow is a coherent link. Providers are the final authority; the backend is authoritative for everything below it; the runtime is authoritative for the client; the client is authoritative for nothing but is where the user acts.
