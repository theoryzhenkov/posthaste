...
## 10. Deployment instances

The same mechanism appears at every boundary; deployment chooses placement and transport.

- **Bundled application.** The runtime is an embedded authority co-located with the renderer. The runtime↔backend link is in-process; the client↔runtime link is in-process/IPC. The renderer holds no outbox because its authority is local and instant; the client-layer link node *is* the embedded runtime.
- **Hosted runtime / multi-device.** The runtime is remote. The client↔runtime link runs over a remote transport, so the **client layer is a full link node** with its own outbox, optimistic state, and cache (a replica), running the shared local effect. The renderer remains a pure view over that client-layer node.
- **Hosted backend.** The backend is remote from the runtime; the runtime↔backend link runs over a remote transport with the runtime as a buffering near node.
