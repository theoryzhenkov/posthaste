...
depends:
  - path: docs/runtime/internals/L1
    section: "2. Authority server handle shape"
...
The authority server builder accepts transport-neutral inputs and returns a
...
| multi-window-shared-runtime | MUST | Multiple UI sessions observe the same authority server state through the shared event bus. |
