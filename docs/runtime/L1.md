---
scope: L1
summary: "Runtime roles and bundled application deployment: embedded authority runtime, stateful view operation, instant actions, replica transparency, and multi-runtime fan-in. Shared contract over the adapter, mutations, and internals sub-domains"
modified: 2026-07-01
reviewed: 2026-07-01
state: realized
depends:
...
- **[adapter](adapter/L1.md)** — the renderer-facing surface: sessions, views, descriptors, the `RuntimeFrame` stream, mutation submission/settlement, resources, and the view-operation flow that produces served state.
- **[mutations](mutations/L1.md)** — the named-mutation pipeline and catalog: argument rules, the shared local-effect predictor, the per-action catalog, settlement, and reconciliation.
- **[internals](internals/L1.md)** — assembly and guts: the contract/authority crates, the runtime handle and builder, caller context, startup/shutdown, event/state streams, and security & storage.
...
- **Hosted backend, multi-runtime**: several runtime near nodes share one remote backend, each with its own `RuntimeId`; settlement is routed per runtime while base assertions broadcast ([replication L1 §10](../replication/L1.md), [backend-link L1 §3.1](../replication/backend-link/L1.md)). The runtime's UI-facing contract is unchanged — it is one near node among siblings.
...
| multi-runtime-contract-unchanged | MUST | In a multi-runtime hosted-backend deployment the runtime's UI-facing contract (sessions, views, mutations, settlement, coverage) is unchanged; only the backend far node gains per-runtime identity, idempotency, and settlement routing. |
