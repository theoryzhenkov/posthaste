...
One local authority server serves all renderer windows in the application process tree. Focused windows, compose windows, settings windows, and the main window are UI sessions attached to the same runtime state.
...
- **Bundled**: an embedded authority server co-located with the renderer; links are in-process/IPC; the renderer needs no outbox because its authority is local.
...
| multi-runtime-contract-unchanged | MUST | In a multi-runtime hosted-authority-server deployment the runtime's UI-facing contract (sessions, views, mutations, settlement, coverage) is unchanged; only the authority-server far node gains per-runtime identity, idempotency, and settlement routing. |
