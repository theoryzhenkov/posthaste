...
Endpoint semantics belong to this domain; the per-group behavior and handler patterns are detailed in [L3](L3.md). Backend docs describe how the server routes and implements the contract.
...
Message list endpoints accept `q` as search query text, evaluated relative to the endpoint's scope; `/messages/search` requires `q` and searches across sources. Result membership is backend-evaluated, not client-derived. Invalid query text returns `invalid_query`. The scope-alias equivalences (`in:<source>/<mailbox>`, smart-mailbox and conversation scopes, global search) are detailed in [L3 §3](L3.md).

## 8. Events

### 8.1 Event sequence

`GET /events` opens either a live Server-Sent Events stream or a finite SSE response. Event sequence numbers come from backend event history. Clients use the last applied sequence number as their resume cursor. Domain-event frames have `id` set to the backend event sequence number and `data` set to the JSON-serialized `DomainEvent`.
...
When `afterSeq` is provided without `collapse`, the backend replays matching events from event history before switching to live events, so clients reconnect without missing ordered state.
