---
title: "API — Implementation reference (L3)"
scope: L3
summary: "API implementation reference and patterns: the standards the contract draws on, per-group endpoint semantics and handler behavior (accounts, settings, automation-rules CRUD, smart mailboxes, messages, compose + idempotency, sync, reads, tokens), search-query scope semantics, non-renderer client consumers, the contract-artifact change checklist, and the validation/test patterns."
modified: 2026-07-04
reviewed: 2026-07-04
state: realized
depends:
...
6. regenerating event types when AsyncAPI changes — the web client's `EventTopic` union in `apps/web/src/api/events.gen.ts` is generated from `asyncapi.json` by `apps/web/scripts/gen-event-topics.ts` (`bun run events:generate`) and drift-checked by `events:check`
