...
Bundled application renderers may use an in-process or IPC runtime adapter instead of `/v1` for primary mail state; the adapter may use generated API types internally, but renderer semantics come from the runtime contract. Remote renderers, hosted clients, or future adapters use `/v1` directly when no local runtime adapter is available. The role binaries these consumers attach to are, end-state: `posthaste-authority-runtime-server` (the bundled all-in-one), `posthaste-authority-server` (the standalone far node), `posthaste-runtime` (the near-node daemon), and `posthaste-client` (the desktop app).
