---
scope: L2
summary: "Authority-server component structure: the authority-server-tier crates (the workspace-wide topology is in architecture/L2-crate-topology), runtime assembly, services, providers, store, and config/secrets (operational flows and the /v1 adapter patterns are in L3)"
modified: 2026-07-02
reviewed: 2026-07-02
state: realized
depends:
  - path: docs/architecture/L2-crate-topology
    local: "1. Crate structure"
  - path: docs/state/mail/L2
    section: "2. Authority-server storage shape"
...
The workspace-wide crate set, hierarchy, and wasm frontier are named once in
[Crate topology](../architecture/L2-crate-topology.md). This section keeps only
the authority-server-tier detail:
...
| `posthaste-authority-server` | The far (authoritative) node of the runtime↔authority-server link: authority-server node assembly over config, secrets, SQLite store, domain service, and provider gateways; account supervisor; sync; push; OAuth flow state; `AuthorityServerApi` implementations; and its own link wire (`link_router` + link auth — the far node owns the surface it serves). No re-exports of near-node symbols. |
...
The runtime crate (`posthaste-runtime`) assembles one runtime handle — the near node — before any transport adapter is built; the far node it links to is assembled by `posthaste-authority-server` ([Crate topology §1.4](../architecture/L2-crate-topology.md)). The handle is the process-local implementation of the shared runtime contracts (`posthaste-runtime-api` for typed wire-free domain RPC, `posthaste-client-link` for session/view/frame/mutation-forward link ops) for mail behavior.
...
Bundled startup resolves platform paths, builds the runtime handle, starts enabled account runtimes, registers the renderer runtime adapter over the handle, and opens renderer windows. The Tauri adapter uses the handle's session, view, mutation, resource, and event methods; it does not call Axum handlers for primary renderer mail behavior, except while a contained loopback bridge is intentionally used during migration.
