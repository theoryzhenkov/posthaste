---
scope: L3
summary: "Authority-server implementation detail: how the /v1 Axum adapter implements the API contract over the RuntimeHandle (handler patterns, error mapping, route guard), the runtime/sync/mutation/event flows, and the extension points for new providers, reads, mutations, and events. The wrapper migration is complete; its temporary helpers are scheduled for removal."
modified: 2026-07-02
reviewed: 2026-07-02
state: realized
depends:
...
migration that introduced this seam is complete; its residue (the temporary
`api_bridge`/`runtime_handle_*_for_migration` helpers) is scheduled for removal
(RFC-L2-architecture-cleanup, migration-residue decision). This spec records
the realized adapter shape.
...
The concrete `AccountSupervisor` and gateway ownership live in `posthaste-authority-server`; `posthaste-server::supervisor` is a compatibility re-export for tests and remaining adapter imports only. This keeps Axum handlers out of gateway lookup and makes the account-runtime graph transport-free for embedded desktop reuse.
