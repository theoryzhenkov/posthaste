...
use posthaste_authority_server_link::{
    message_mutation::MessageMutation, AuthorityServerFrame, AuthorityServerLinkId, WireMutationId,
...
impl AuthorityServer {
    ...
    /// Apply one named message mutation — the authority server's up-channel handler. This
    /// is the dispatch from a transport-neutral named mutation
    /// (`message.setKeywords` / `message.moveToRole` / …) to the typed command,
    /// moved here from the runtime: the authority server "accepts named mutations"
    /// ([replication authority-server-link L1 §3](../replication/authority-server-link/L1.md)). The runtime keeps the
