pub(super) const SCHEMA_SQL: &str = "
            ...
            -- NS1: the incremental mailbox-counter triggers
            -- (mailbox_counters_message_mailbox_ai/_ad, _message_read_au) are
            -- RETIRED — list_mailboxes derives counts live over the
            -- `*_effective` views, so there is no materialized counter to
            -- maintain (or to drift, DP-H12). init_schema DROPs the old
            -- triggers on legacy databases; mailbox.unread_emails/total_emails
            -- are dead columns pending the schema-versioning migration (M84).
