...
//!
//! `crate::tests::run_outputs`, `crate::tests::run_statuses`, and
//! `crate::tests::cli_help` each used to hand-roll their own
//! `std::env::temp_dir()` + unique-naming helper and never clean up the
//! directory. All now share [`temp_root`] from here instead.
//!
//! Backed by [`tempfile::TempDir`] rather than reinventing unique-name
//! generation and recursive removal. [`TempDirGuard`] adds a
//! `Deref<Target = Path>` so existing `let root = temp_root(); ...
