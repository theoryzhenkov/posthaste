...
impl MailService {
    ...
    /// Test-only: production overlay maintenance filters inline in
    /// `replay::refresh_message_overlay` (the one lifecycle function); this
    /// projection is retained for lifecycle-assertion tests.
    #[cfg(test)]
