use posthaste_domain_service::{AccountId, ProviderKind, ProviderProfile, SyncBatch};
use posthaste_imap::{
...
pub(super) fn imap_sync_batch() -> SyncBatch {
    let selected = posthaste_domain_service::ImapSelectedMailbox {
        mailbox_id: posthaste_imap::imap_mailbox_id("INBOX"),
        mailbox_name: "INBOX".to_string(),
        uid_validity: posthaste_domain_service::ImapUidValidity(9),
        uid_next: None,
        highest_modseq: None,
    };
    let mapped = imap_header_message_record(
        ...
        ImapFetchedHeader {
            ...
            uid: posthaste_domain_service::ImapUid(42),
            modseq: Some(posthaste_domain_service::ImapModSeq(777)),
...
pub(super) fn imap_gmail_label_sync_batch() -> SyncBatch {
    let inbox = posthaste_domain_service::ImapSelectedMailbox {
        mailbox_id: posthaste_imap::imap_mailbox_id("INBOX"),
        mailbox_name: "INBOX".to_string(),
        uid_validity: posthaste_domain_service::ImapUidValidity(11),
        uid_next: None,
        highest_modseq: Some(posthaste_domain_service::ImapModSeq(900)),
    };
    let archive = posthaste_domain_service::ImapSelectedMailbox {
        mailbox_id: posthaste_imap::imap_mailbox_id("[Gmail]/All Mail"),
        mailbox_name: "[Gmail]/All Mail".to_string(),
        uid_validity: posthaste_domain_service::ImapUidValidity(12),
        uid_next: None,
        highest_modseq: Some(posthaste_domain_service::ImapModSeq(901)),
    };
    let inbox_header = imap_label_header(&inbox, posthaste_domain_service::ImapUid(101), &["\\Seen"]);
    let archive_header = imap_label_header(&archive, posthaste_domain_service::ImapUid(202), &["\\Seen"]);
    ...
    imap_full_sync_batch(
        ...
        posthaste_imap::DiscoveredImapAccount {
            capabilities: posthaste_domain_service::ImapCapabilities::from_tokens([
                "IMAP4rev1",
                "CONDSTORE",
                "QRESYNC",
                "X-GM-EXT-1",
...
pub(super) fn imap_gmail_flagged_delta_batch(
    locations: Vec<posthaste_domain_service::ImapMessageLocation>,
) -> SyncBatch {
    let archive = posthaste_domain_service::ImapSelectedMailbox {
        mailbox_id: posthaste_imap::imap_mailbox_id("[Gmail]/All Mail"),
        mailbox_name: "[Gmail]/All Mail".to_string(),
        uid_validity: posthaste_domain_service::ImapUidValidity(12),
        uid_next: None,
        highest_modseq: Some(posthaste_domain_service::ImapModSeq(1102)),
    };
    let changed_header = imap_label_header(
        &archive,
        posthaste_domain_service::ImapUid(202),
        &["\\Seen", "\\Flagged"],
    );
    ...
    imap_condstore_delta_sync_batch(
        ...
        vec![changed_header],
        vec![posthaste_domain_service::ImapMailboxSyncState {
            mailbox_id: archive.mailbox_id.clone(),
            mailbox_name: archive.mailbox_name,
            uid_validity: archive.uid_validity,
            highest_uid: Some(posthaste_domain_service::ImapUid(202)),
...
fn imap_label_header(
    selected: &posthaste_domain_service::ImapSelectedMailbox,
    uid: posthaste_domain_service::ImapUid,
    flags: &[&str],
    ...
    imap_header_message_record(
        ...
        ImapFetchedHeader {
            mailbox_id: selected.mailbox_id.clone(),
            uid,
            modseq: Some(posthaste_domain_service::ImapModSeq(900 + uid.0 as u64)),
