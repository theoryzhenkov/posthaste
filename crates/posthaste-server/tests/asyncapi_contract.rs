...
//! `posthaste_domain_model::ALL_EVENT_TOPICS` (the single source of truth). Drift here
...
fn asyncapi_topics_match_authority_server_constants() {
    ...
    let constants: BTreeSet<String> = posthaste_domain_model::ALL_EVENT_TOPICS
        .iter()
        .map(|s| s.to_string())
        .collect();
    ...
    assert!(
        ...
        "topics documented in asyncapi.json are not in posthaste_domain_model::ALL_EVENT_TOPICS: {missing_from_constants:?}"
    );

    assert_eq!(
        documented, constants,
        "asyncapi.json EventTopic enum has drifted from posthaste_domain_model::ALL_EVENT_TOPICS"
    );
}
