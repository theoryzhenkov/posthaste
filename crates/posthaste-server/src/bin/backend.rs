//! `posthaste-backend`: the standalone backend role binary (L5 §4).
//!
//! Builds the backend far node (store + service + account supervisor) and serves
//! ONLY the authenticated runtime↔backend link — no `/v1` client API, no
...
async fn main() {
    ...
    let handle = start_backend(ServerConfig {
    ...
    handle
        .join_handle
        .await
        .expect("posthaste-backend task panicked");
}
...
fn usage() -> &'static str {
    "usage: posthaste-backend [--bind 127.0.0.1:3002]"
}
