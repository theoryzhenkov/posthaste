...
//! near-node assertion fold, authority server). This module centralizes that
...
//! - Far node: authority server command execution (kept in `authority-server/authority_server.rs`
//!   because it uses internal `AuthorityServer` methods).
...
impl MessageMutation {
    ...
    /// view before the authority server confirms. `None` for role moves when no role map
