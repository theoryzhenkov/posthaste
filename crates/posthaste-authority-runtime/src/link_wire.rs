...
//! drag the `/v1` client platform (`posthaste-api`) to serve it. The wire's
...
impl LinkAuth {
    ...
    /// mandatory — absent or empty ⇒ panic (the backend is network-exposed; a
    ...
    /// context (e.g. `"posthaste-backend"`, `"[link] serve"`). The fail-closed
...
/// client (`RemoteBackend`) sees the same response it does across the
