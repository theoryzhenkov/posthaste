...
# the POSTHASTE_AUTHORITY_RUNTIME_SERVER_NAME env). Used for the distributed self-host binaries:
# posthaste-authority-server, posthaste-runtime, and the posthaste-wizard
...
# Env:
#   POSTHASTE_BIN              - cargo binary name, e.g. posthaste-authority-server
#   POSTHASTE_ARTIFACT_NAME    - release artifact base name, e.g. PosthasteAuthorityServerNightly
...
bin_name="${POSTHASTE_BIN:?POSTHASTE_BIN is required (cargo bin name, e.g. posthaste-authority-server)}"
