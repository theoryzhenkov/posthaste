...
#
# Requires: bun --cwd=apps/client/frontend run build already run
# (apps/client/frontend/dist populated).
# Env:
...
if [[ ! -f "$root/apps/client/frontend/dist/index.html" ]]; then
  echo "missing apps/client/frontend/dist/index.html; run 'bun --cwd=apps/client/frontend run build' first" >&2
  exit 1
fi

rm -rf "$out_dir"
mkdir -p "$out_dir"
cp -R "$root/apps/client/frontend/dist" "$out_dir/web"
