...
export interface FieldInfo {
  ...
  name: string;
  /** Kebab-case flag name (`sourceId` → `source-id`). */
  flag: string;
  kind: FieldKind;
...
function kindOf(schema: z.ZodTypeAny): FieldKind {
  ...
  // ZodString, ZodEnum, ZodLiteral, and anything else are scalar strings at the
  // CLI boundary (zod still validates the concrete constraint).
  return "string";
}
...
export function describeFields(shape: ArgShape): FieldInfo[] {
  return Object.entries(shape).map(([name, schema]) => {
    ...
    return {
      ...
      elementKind: kind === "array" ? elementKindOf(typed) : undefined,
    };
  });
}
