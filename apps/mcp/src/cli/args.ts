...
function coerceField(values: string[], field: FieldInfo): unknown {
  if (field.kind === "array") {
    ...
    // --flag b` collects into a string array (with scalar element coercion).
 ...
 * from `=value`, the next token, or (for booleans) implied `true`. An optional
 * JSON `inputObject` (from `--input`) seeds the args; explicit flags override it;
 * a single bare positional fills the operation's `primary` field.
 */
