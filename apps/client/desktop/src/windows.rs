//! Window glue: the window factory (with connection-info injection and
//! external-link routing), webview boot ACKs, and the close-request routing
//! behind the Close Window menu item.
...
use crate::injection::{backend_init_script, BackendInjection, FocusedWindowLabel};
use crate::surfaces::{
    surface_route, surface_title, surface_window_label, surface_window_navigation_script,
    surface_window_size, surface_window_url, validate_surface_descriptor, SurfaceDescriptor,
};
use crate::{CLOSE_WINDOW_REQUESTED_EVENT, MAIN_WINDOW_LABEL};
...
/// An `async` command runs on the async runtime thread pool, leaving the
/// main thread free to pump the creation to completion.
#[tauri::command]
pub(crate) async fn open_surface_window(
    ...
    let backend = {
        let backend = app.state::<crate::backend::EmbeddedBackend>();
        BackendInjection {
            port: backend.port,
...
/// "Developer tools" setting: the frontend only invokes it when that flip is
/// on (and binds the shortcut), giving a single build with toggleable
/// devtools. A no-op when devtools are not compiled in.
...
pub(crate) fn build_window<M: Manager<R>, R: Runtime>(
    ...
    // lights sit inside the app UI; the frontend paints the matching drag
    // region + inset (see `isMacDesktop` in apps/client/frontend/src/desktop.ts).
    #[cfg(target_os = "macos")]
    let builder = builder
