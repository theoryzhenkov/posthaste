 ...
 *
 *  - NEW ARRIVALS ONLY: `payload.changes.arrived === true` AND
 *    `payload.created === true`. A user's own mutation echo (mark-read, tag
 *    flip) carries no `arrived` flag; moving an EXISTING message into a
 *    mailbox flips `arrived` but not `created` — neither notifies.
 ...
 *    (`$draft` — a saved draft) is skipped.
 *  - Never during an account's INITIAL sync: while the account has never
 *    completed a sync (`lastSyncAt == null` on its `accounts` row), every
 *    "arrival" is backfill, not news.
 *  - BURST-COALESCED: arrivals collect in a sliding debounce window (capped
 *    at {@link NEW_MAIL_MAX_COALESCE_MS} total deferral so a steady trickle
 *    still flushes); one flush posts ONE banner — a single sender + subject,
 *    or an "N new messages" summary.
 ...
 *  - TOGGLE-GATED: the notification settings' `newMail` toggle (default ON)
 *    gates delivery; the `sound` toggle rides along on the posted banner.
 *
 * The coordinator is pure policy with an injected notifier so the gate is
 * unit-testable. The event payload is a PROMPT only: it decides whether to
 * banner, and is never folded into anything the UI renders from —
 * {@link useNewMailNotifications} is the production wiring on the stream.
 */
...
export interface NewMailArrivalCoordinator {
  onMessageUpdated: (event: DomainEventPayload) => void
...
// ---------------------------------------------------------------------------
// Production wiring (the `message.updated` stream prompt).
// ---------------------------------------------------------------------------
...
function defaultDeps(queryClient: QueryClient): NewMailArrivalDeps {
  return {
    ...
    // Reads of the mirror's current answers — prompts may consult what the
    // queries already said, they never write it.
    getPreferences: () =>
      queryClient.getQueryData<AppSettingsResult>(queryKeys.appSettings)
        ?.settings.notifications,
    isAppFocused: isAppWindowFocused,
    isAccountInInitialSync: (accountId) => {
      const accounts = queryClient.getQueryData<AccountsResult>(
        queryKeys.accounts,
      )
      const account = accounts?.rows.find((entry) => entry.id === accountId)
      ...
      return account ? account.lastSyncAt === null : false
    },
  }
}
...
export function useNewMailNotifications(): void {
  ...
  useDomainEvent('message.updated', (payload) =>
    notifyNewMailFromEvent(queryClient, payload),
  )
}
