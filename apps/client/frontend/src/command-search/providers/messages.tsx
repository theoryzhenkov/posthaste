...
/** One free-text search window against the `mailList` family. */
...
export function createMessageProvider(input: {
  ...
  /** Evaluates the palette's free-text window via the `mailList` family. */
  ...
  const messageProvider: SearchProvider = {
    ...
    async search(req: ProviderSearchRequest) {
      ...
      const page = await input.searchMessages({
        freeText: serverQuery,
        cursor: req.cursor ?? null,
        limit: req.limit,
      })
      return {
        candidates: page.rows.map((message, index) =>
          messageCandidate(
            messageProvider,
            message,
            input.readModels.sources,
