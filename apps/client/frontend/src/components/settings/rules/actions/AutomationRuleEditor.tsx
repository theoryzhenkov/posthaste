...
import type { Mailbox, MailQueryRule } from '../../../../data/transport/api/index'
import type { AccountRow, AutomationRulePreviewResult } from '@/gen'
...
import { ConditionEditorContext } from '../conditions/conditionEditorContext'
import { Field, SettingsBackButton } from '../../panel/shared'
  ...
  const previewMutation = useMutation({
    mutationFn: async (input: { key: string; condition: MailQueryRule }) => {
      const result = await fetchQuery<AutomationRulePreviewResult>(client, {
        automationRulePreview: { condition: input.condition, limit: 5 },
      })
      return {
        key: input.key,
        preview: { total: result.total, items: result.rows },
      }
    },
  })
  const activePreview =
