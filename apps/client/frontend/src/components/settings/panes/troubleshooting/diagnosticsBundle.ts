 ...
 * structural fields (status, push, last sync error) are included, and any
 * free-text error message is run through {@link sanitizeText} (email masking +
 * secret redaction) before inclusion.
 */
import type { AccountRow, AccountStatus, PushStatus } from '@/gen'
import type { ReleaseChannel } from '@/lib/platform/releaseChannel'
...
export function summarizeAccount(account: AccountRow): DiagnosticsAccountSummary {
  return {
    ...
    lastSyncError: account.lastSyncError
      ? compactError(account.lastSyncError)
      : null,
  }
}
  ...
  if (input.accounts.length === 0) {
    ...
    input.accounts.forEach((account, index) => {
      ...
      if (summary.lastSyncError) {
        parts.push(`— last error: ${summary.lastSyncError}`)
      }
