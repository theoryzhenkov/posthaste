...
import { useDiagnosticsBundle } from './useDiagnosticsBundle'
...
export function TroubleshootingPane() {
  const { developerTools } = usePlatformServices()
  const developerToolsEnabled = useStore(developerTools)
      ...
      <SettingsAdvanced label="Developer">
        <div className="grid gap-3 sm:grid-cols-[1fr_280px] sm:items-center">
          ...
          <button
            ...
            onClick={() => developerTools.set(!developerToolsEnabled)}
            className={cn(
