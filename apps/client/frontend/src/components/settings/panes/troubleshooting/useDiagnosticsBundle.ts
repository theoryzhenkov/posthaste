...
import { releaseChannel } from '@/lib/platform/releaseChannel'
...
import { formatDiagnosticsBundle } from './diagnosticsBundle'
...
export function useDiagnosticsBundle(): UseDiagnosticsBundleResult {
  const { diagnostics } = usePlatformServices()
  ...
  const revealLogFolder = useCallback(async () => {
    ...
    try {
      await diagnostics.revealLogFolder()
    } catch (error) {
      syncLogger.warn(
        { event: LOG_EVENTS.diagnosticsCopyFailed, error },
