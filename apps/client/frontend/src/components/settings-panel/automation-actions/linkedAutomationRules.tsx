...
export function LinkedAutomationRuleFields({
  ...
  // The settings document is written whole: fold the changed rule arrays into
  // the freshest document and post one `updateSettings` command.
  const persistMutation = useMutation({
    mutationFn: async (input: {
      ...
      const next: AppSettings = {
        ...current,
        automationRules: input.automationRules ?? current.automationRules,
        automationDrafts: input.automationDrafts ?? current.automationDrafts,
      }
      await runCommand(client, queryClient, {
        updateSettings: {
          settings: next,
          forceBackfill: input.forceBackfill ?? false,
        },
      })
      return next
    },
