...
You need **the Posthaste desktop app running** (PosthasteNightly) _or_ the
...
_same_ event reproduces the _same_ key — see
...
handler can see the same message twice. That is safe _only if your write-back is
idempotent_. Pass an **`Idempotency-Key`** header on any `apply` (message
command) request:
...
authored in a `rules.toml` file in your config root, runs _inside_ the
...
A `webhook` action needs _something_ listening at its URL. For a
...
- `--token <token>` — require `Authorization: Bearer <token>` on every
  ...
  this receiver. It gates _who may call the listener_, not a substitute for
...
(RFC-L2-scripting ruling 20e): it only filters what your local script _sees_ —
...
authority server** provides the _trigger_, and a **localhost agent connected
over MCP** provides the _capability_ (typed tools + a live event feed).
...
`notifications/message`. But delivery is not the same as _waking_ your agent.
...
- **Wake + capability:** to have a server-side event _trigger_ the agent
  ...
  it capability. `emit` + MCP-notifications is the elegant path _only_ once the
...
server _pushes_ `rule.fired`; this loop turns that push into work (a stock chat
...
client.setNotificationHandler(LoggingMessageNotificationSchema, async (n) => {
  ...
  if (d.kind === 'gap') {
    /* reconcile: re-read state */ return
  }
...
pattern) or your own supervisor. If you would rather the _server_ invoke your
