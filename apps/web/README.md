...
- `src/components/Sidebar.tsx` renders smart mailboxes, tags, and enabled account mailboxes from normalized React Query caches; `src/mailboxNavigationReadModels.ts` bootstraps them with a client-composed `POST /read` operation.
...
- `src/hooks/useDaemonEvents.ts` opens the EventSource connection, persists the last seen event sequence in `sessionStorage`, dispatches browser `CustomEvent`s for list-level live updates, and batches React Query invalidations for normalized navigation and detail data.
...
- Normalized navigation, message detail, and selected message caches are invalidated in short batches.
