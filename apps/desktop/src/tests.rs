...
#[test]
fn release_channel_command_reports_baked_channel() {
    // The renderer reads the channel via this command; it must equal the
    // compiled-in constant so the web bundle and binary cannot disagree.
    assert_eq!(release_channel(), RELEASE_CHANNEL);
    // build.rs always resolves a non-empty channel (defaults to "dev").
    assert!(
        !RELEASE_CHANNEL.is_empty(),
        "RELEASE_CHANNEL must be non-empty"
    );
}
